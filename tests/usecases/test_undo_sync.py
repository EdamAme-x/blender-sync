...
def test_undo_force_flag_makes_next_tick_force_packets():
    """User pressed Ctrl+Z. Gateway raised the flag and pre-marked
    dirty. SyncTickUseCase must build force=True packets so peers
    accept the rewound state."""
    ...
    # Both packets must carry force=True. We re-decode to inspect.
...
def test_undo_flag_consumed_once_per_undo():
    ...
    packets. Only the first post-undo tick should be force=True."""
    uc, scene, transport, session = _make_uc()
    ...
    scene.dirty[CategoryKind.TRANSFORM] = [{"n": "Cube", "loc": [1, 0, 0]}]
    uc.tick(session)
...
def test_undo_force_packet_uses_reliable_seq_counter():
    ...
    # 2 packets sent. MATERIAL is RELIABLE; TRANSFORM is FAST.
    # Both go out as force=True, but TRANSFORM still rides the
    # unreliable counter (force flag doesn't override channel).
    decoded = [_JsonCodec().decode(d) for _, d in transport.sent]
    assert all(p.force for p in decoded)
    # Reliable counter advanced for the MATERIAL force packet.
    assert rel_seq.current == 1
    # Unreliable counter advanced for the TRANSFORM force packet.
