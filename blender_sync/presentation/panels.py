...
class SYNC_PT_main(Panel):
    ...
    def draw(self, context):
        ...
        box = layout.box()
        box.label(text=_("Status: %s") % _(state.status))
        if state.error:
        ...
        if state.token:
            ...
            col2.label(text=_("Share Token:"))
            col2.prop(state, "token", text="")
            col2.operator("blender_sync.copy_token", icon="COPYDOWN")

        layout.separator()
        col3 = layout.column(align=True)
        col3.label(text=_("Join Existing:"))
        col3.prop(state, "join_token", text="")
        col3.operator("blender_sync.join_session", icon="LINKED")

        if state.status == "awaiting_manual_answer":
            layout.separator()
            box4 = layout.box()
            box4.label(text=_("Manual SDP fallback active"), icon="ERROR")
            col_help = box4.column(align=True)
            ...
            box4.separator()
            box4.prop(state, "manual_answer_input", text="")
            box4.operator("blender_sync.submit_manual_answer", icon="IMPORT")
